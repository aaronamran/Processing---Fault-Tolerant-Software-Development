public class Bildverarbeitung
{
    ...
    public int[][] holeRotbild()
    {
        ...
        if(pix!=null)
        {
            ...
            for(int i=0;i<bild.length;i++)
            {
                for(int k=0;k<bild[i].length;k++)
                {
                    ...
                    if(bildR[i][k]<0) bildR[i][k]=-bildR[i][k]; else bildR[i][k]=0;
                    bildG[i][k] = 2*GRUEN - BLAU - ROT + ANHEBUNG;
                    if(bildG[i][k]<0) bildG[i][k]=-bildG[i][k]; else bildG[i][k]=0;
                    bildB[i][k] = 2*BLAU - ROT - GRUEN + ANHEBUNG;
                    if(bildB[i][k]<0) bildB[i][k]=-bildB[i][k]; else bildB[i][k]=0;
      ...
      return bildR; //ROTES Bild zeigen
